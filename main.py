...
class CoxaCheckIn:
    ...
    def lets_checkin(self) -> None:
        ...
        logging.info("Site do Coritiba aberto!")
        time.sleep(1)

        login_success = self.login_socios_page()
        if login_success:
            logging.info("Login realizado com sucesso, seguindo para o check-in")
    ...
    def select_stadium_sector(self):
        ...
        if sector_to_sit == "arquibancada":
            ...
            logging.info("Realizado check-in na Arquibancada!")
            time.sleep(1)
            ...
            logging.info("Realizado check-in na Mauá!")
            time.sleep(1)
    ...
    def send_email_notification(png_file_path: str):
        ...
        try:
            ...
            server.quit()
            logging.info(f"Email de confirmação enviado com sucesso para {os.environ.get("EMAIL")}:")
